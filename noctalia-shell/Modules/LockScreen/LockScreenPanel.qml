...
Item {
  ...
  // Exposed so other lock screen elements (e.g. auth notifications) can avoid overlapping
  // the system stats gauges.
  readonly property bool systemStatsVisible: bottomContainer.showSystemStats
  readonly property real systemStatsExtraHeight: bottomContainer.systemStatsExtraHeight
  ...
  Rectangle {
    ...
    // System stats gauges above the password field (non-compact lock screen only).
    // Opt-out with Settings.data.general.lockScreenShowSystemStats = false.
    readonly property bool showSystemStats: !Settings.data.general.compactLockScreen && (Settings.data.general.lockScreenShowSystemStats !== false)
    readonly property real systemStatsExtraHeight: lockSystemStats.implicitHeight + Style.marginL
    ...
    height: {
      ...
      if (bottomContainer.showSystemStats) {
        calcHeight += bottomContainer.systemStatsExtraHeight;
      }
      return calcHeight;
    }
    anchors.horizontalCenter: parent.horizontalCenter
    ...
    ColumnLayout {
      ...
      // System stats gauges (CPU / temp / RAM) + network speeds, above the password field
      LockScreenSystemStats {
        id: lockSystemStats
        Layout.fillWidth: true
        visible: bottomContainer.showSystemStats
      }
