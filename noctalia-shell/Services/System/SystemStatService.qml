...
Singleton {
  ...
  // While the lock screen is up, only poll when the lock screen itself asked for stats
  // (e.g. the lock screen system-stat gauges). Other consumers stay paused to save power.
  readonly property bool _lockScreenConsumerActive: _registered["lockscreen"] !== undefined
  readonly property bool shouldRun: _registeredCount > 0 && (!_lockScreenActive || _lockScreenConsumerActive)
